...
| Exness标的 | 参考盘口 | 每单位盘口数量对应的标的数量 |
| --- | --- | --- |
| BTCUSD | 币安BTCUSDT U本位永续 | 1 BTC |
| ETHUSD | 币安ETHUSDT U本位永续 | 1 ETH |
...
```text
名义仓位 N = 风险预算 ÷（止损距离% ÷ 100）
参考数量 = N ÷（参考盘口中价 × 参考合约乘数）
逐档冲击 = 2 × max(买单相对买入一档的额外成本, 卖单相对卖出一档的额外成本)
近似总成本 = 基础费用 + 逐档冲击 + 手动额外滑点 - 假设返佣
成本占风险% = 近似总成本 ÷ 风险预算 × 100
止损总亏 = 风险预算 + 近似总成本
```
...
- 结果显示盘口来源、源报价时间、基础费用/参考冲击拆分以及双侧使用档数；5%/10%颜色规则沿用其他平台。移除旧版“线性反推最小止损”，因为冲击随仓位变化，不能沿用固定费率公式倒推。
- 默认每3秒获取选中参考盘口；Bybit复用单个订阅连接。请求超时退避，同一请求去重，切换品种/平台取消旧请求；报价超过15秒、网络失败、休市或深度不足时清空结果并禁止复制，不静默回退成无冲击结果。暂停/后台保留的快照也会到期。
- **近似边界**：币安USDT按1美元换算；用等名义金额映射，不认为两个平台的真实流动性相等。未计Exness手数取整、单笔下单限制、隔夜、未来跳空；不代表Exness可成交容量。程序不连接交易账户、不下单。
